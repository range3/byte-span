...
TEMPLATE_TEST_CASE(
    "basic_byte_span construction from std::array - byte-like types",
    "[basic_byte_span][array][template]",
    char,
    unsigned char,
    std::byte) {
  using t = TestType;
  ...
  SECTION("non-const array - implicit conversion") {
    const basic_byte_span span = arr;  // implicit CTAD
  ...
  SECTION("const array - const result") {
    const basic_byte_span span = const_arr;  // implicit CTAD
    REQUIRE(span.size() == sizeof(t) * 3);
    REQUIRE(span.data()
  ...
  SECTION("const elements - const result") {
    const basic_byte_span span = const_elem_arr;  // implicit CTAD
    REQUIRE(span.size() == sizeof(t) * 3);
    REQUIRE(span.data()
  ...
  SECTION("const array and elements - const result") {
    const basic_byte_span span = const_all_arr;  // implicit CTAD
    REQUIRE(span.size() == sizeof(t) * 3);
    REQUIRE(span.data()
...
TEMPLATE_TEST_CASE(
    "basic_byte_span construction from std::array - non-byte types",
    "[basic_byte_span][array][template]",
    int,
    double,
    trivial_struct) {
  ...
  SECTION("non-const int array - explicit conversion required") {
    STATIC_REQUIRE_FALSE(
        std::is_convertible_v<std::array<int, 3>&, basic_byte_span<std::byte>>);

    // explicit conversion required
    auto const span = basic_byte_span{arr};  // CTAD
  ...
  SECTION("const int array - const result") {
    auto const span = basic_byte_span{const_arr};  // CTAD
  ...
  SECTION("const elements - const result") {
    auto const span = basic_byte_span{const_elem_arr};  // CTAD
...
TEST_CASE("basic_byte_span construction from std::array - size checks",
          "[basic_byte_span][array]") {
  auto arr = std::array<int, 3>{1, 2, 3};

  SECTION("dynamic extent") {
    auto const span = basic_byte_span<std::byte>{arr};
    REQUIRE(span.size() == sizeof(int) * 3);
    STATIC_REQUIRE(span.extent == lsm::utils::dynamic_extent);
  }

  SECTION("static extent") {
    auto const span = basic_byte_span<std::byte, sizeof(int) * 3>{arr};
    REQUIRE(span.size() == sizeof(int) * 3);
  ...
  SECTION("zero extent") {
    ...
    auto const span = basic_byte_span{empty_arr};  // CTAD
